...
app.put('/api/fetch-message', async (req, res) => {
    ...
    try {
        if (!hi) {
            return res.status(400).json({ success: false, message: 'Please provide valid content.' });
        }
        
        // Fixed: Replaced undefined variable "name" with the incoming variable "hi"
        const greeting = `Hello, ${hi}! Welcome back.`; 
        
        return res.json({ success: true, message: 'Process successful!', note: greeting });
